...
#define PEACHY_TAB_STOP 8
#define PEACHY_QUIT_TIMES 2
...
int editorRowCxToRx(erow *row, int cx) {
	...
	for (int j = 0; j < cx; j++) {
		if (row->chars[j] == '\t')
			rx += (PEACHY_TAB_STOP - 1) - (rx % PEACHY_TAB_STOP);
		rx++;
	}
	return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
	...
	for (cx = 0; cx < row->size; cx++) {
		if (row->chars[cx] == '\t')
			cur_rx += (PEACHY_TAB_STOP - 1) - (cur_rx % PEACHY_TAB_STOP);
		cur_rx++;
		if (cur_rx > rx)
			return cx;
	}
...
void editorUpdateRow(erow *row) {
	...
	free(row->render);
	row->render = malloc(row->size + tabs * (PEACHY_TAB_STOP - 1) + 1);

	int idx = 0;
	for (int j = 0; j < row->size; j++) {
		if (row->chars[j] == '\t') {
			row->render[idx++] = ' ';
			while (idx % PEACHY_TAB_STOP != 0)
				row->render[idx++] = ' ';
		} else {
...
void editorProcessKeypress(void) {
	static int quit_times = PEACHY_QUIT_TIMES;
